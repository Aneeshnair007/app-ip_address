 ...
 * @return {string} (Ipv6Address1) - An Ipv6Address1 address.
  * @return {object} - An object containing both an IPv4 address an its mapped ipv6 address.
 */
function getFirstIpAddress(cidrStr, callback) {
  ...
  if (!cidr.isValid()) {
    ...
    [firstIpAddress] = cidr.toArray(options);
     ...
     let Ipv6Address1 = getIpv4MappedIpv6Address(firstIpAddress);
    
    obj = {  "ipv4": firstIpAddress, "ipv6": Ipv6Address1};
...
function main() {
  ...
  for (let i = 0; i < sampleCidrsLen; i++) {
    ...
    getFirstIpAddress(sampleCidrs[i], (data, error) => {
      ...
      //console.log(`  Response returned from GET request: ${data.toString}`);
      console.log(`  Response returned from GET request: `,JSON.parse(JSON.stringify(data)));
    });
  }
